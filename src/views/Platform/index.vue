<template>
  <session>
    <div class="query">
      <el-form :form="queryForm" label-width="50px" @submit.prevent inline>
  ...
  <session>
    <HModel>
      <template #head>
        <span style="font-weight: bolder">{{ $t('menu.platform') }}</span>
        <div>
      ...
      <template #body>
        ...
        >
          <el-table-column label="ID">
            <template #default="{ row }">{{ row.game_pingtai_id }}</template>
          </el-table-column>
...
const tableData = ref<platformList>()
...
// 编辑
function editCate(row: platformList) {
  operation.value = '编辑'
  setTimeout(() => {
  ...
  dialogVisible.value = true
  editId.value = row.game_pingtai_id
}
// 弹出框确定
...
async function delCate(row: platformList) {
  const res: any = await delPlatform({ ids: [row.game_pingtai_id] })
  if (res.code === 200) {
